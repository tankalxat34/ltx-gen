	...
	def window(root_name, prj_name, status_fill):
		...
		def Analyze(root_name, prj_name):
			...
			if sys_outfit_name!='':
				...
				ways_player_ogf=os.listdir(ogf_player_folder)
				ways_dds=os.listdir(dds_folder)

				print(ways_ground_ogf)
				print(ways_player_ogf)
				print(ways_dds)

				#if len(ways_dds)<=3 and len(ways_ground_ogf)==1 and len(ways_player_ogf)==1:

				try:

				# СПИСОК, ИЗ КОТОРОГО ДОЛЖНЫ ПРОВОДИТСЯ ИСКЛЮЧЕНИЯ: ЕСЛИ ЗАГРУЖЕННЫЙ ФАЙЛ НЕ В СПИСКЕ ТО ПЕРЕИМЕНОВЫВАТЬ
				# ['act_himerr.dds', 'act_izlo1.dds', 'act_izlom.dds', 'act_izlom_bump#.dds', 'act_izlom_bump.dds', 'act_zombie_1.dds', 'act_zombie_1_bump#.dds', 'act_zombie_1_bump.dds', 'act_zombie_2.dds', 'act_zombie_2_bump#.dds', 'act_zombie_2_bump.dds', 'act_zombie_3.dds', 'act_zombie_4.dds', 'act_zombie_4_bump#.dds', 'act_zombie_4_bump.dds']
				# ['act_himerr.dds', 'act_izlo1.dds', 'act_izlom.dds', 'act_izlom_bump#.dds', 'act_izlom_bump.dds', 'act_nauchniy_moblack.dds', 'act_st_exoskel_tank_monolit.dds', 'act_zombie_1.dds', 'act_zombie_1_bump#.dds', 'act_zombie_1_bump.dds', 'act_zombie_2.dds', 'act_zombie_2_bump#.dds', 'act_zombie_2_bump.dds', 'act_zombie_3.dds', 'act_zombie_4.dds', 'act_zombie_4_bump#.dds', 'act_zombie_4_bump.dds']
...
• {4} - Файлы ogf когда надето.

Нажмите "Да" для продолжения, "Нет" для отмены.
'''.format(prj_name, sys_outfit_name, ways_dds, ways_ground_ogf, ways_player_ogf)

					answer=mb.askyesno('Подтверждение действия', text_continue)
					if answer==True:
						did_list_textures=os.listdir(dds_folder) # БРОНЯ
						#print('Подтверждено!')
						#if len(ways_dds)==1:
						if str(rename_status.get())=='False' and len(did_list_textures)!=0: # если переименование текстур отключено
							for i in range(0, len(ways_dds)): # заходим в цикл
								if ways_dds[i] not in did_list_textures: # если загруженный файл отсутствует в спике текстур
									if re.search(re.escape('_bump#'), ways_dds[i]): # переименовываем в зависимости от условий
										os.rename(dds_folder+ways_dds[i], config.folder_projects+'/'+config.folder_projects_soc+'/'+prj_name+'/gamedata/textures/act/act_{0}{1}.dds'.format(sys_outfit_name, '_bump#'))
									elif re.search(re.escape('_bump'), ways_dds[i]):
										os.rename(dds_folder+ways_dds[i], config.folder_projects+'/'+config.folder_projects_soc+'/'+prj_name+'/gamedata/textures/act/act_{0}{1}.dds'.format(sys_outfit_name, '_bump'))
									elif re.search(re.escape(''), ways_dds[i]):
										os.rename(dds_folder+ways_dds[i], config.folder_projects+'/'+config.folder_projects_soc+'/'+prj_name+'/gamedata/textures/act/act_{0}{1}.dds'.format(sys_outfit_name, ''))
						else:	
							for i in range(0, len(ways_dds)): # заходим в цикл							
								if i==0:
									os.rename(dds_folder+ways_dds[i], config.folder_projects+'/'+config.folder_projects_soc+'/'+prj_name+'/gamedata/textures/act/act_{0}{1}.dds'.format(sys_outfit_name, ''))
						...
						os.rename(ogf_ground_folder+ways_ground_ogf[0], ogf_ground_folder+'{0}{1}'.format(sys_outfit_name, '_suit.ogf'))
						os.rename(ogf_player_folder+ways_player_ogf[0], ogf_player_folder+'stalker_{0}.ogf'.format(sys_outfit_name))
		...
		ttk.Button(FrameBtn, text='Продолжить >>>', width=23, command=Rediction_Function_Analyze, state=DISABLED).pack(side=RIGHT)
